...
export class UserService {
  private buildTempUsername() {
    // users.username 字段长度限制为 12，临时用户名需短且唯一性足够
  ...
  async createUserAndVerifycation(
    ...
    const u: UserEntity = await this.userEntity.findOne({
      where: where.length ? where : [{ email }],
    });
    ...
    try {
      ...
      const autoUsername = !userInput.username;
      if (autoUsername) {
        userInput.username = this.buildTempUsername();
      }
      ...
      if (!u) {
        ...
        if (autoUsername) {
          await this.assignSequentialUsername(n.id);
          n.username = `user${n.id}`;
        }
      } else {
        n = u;
      }
  ...
  async updateInfo(body: UpdateUserDto, req: Request) {
    ...
    if (body.username) {
      const taken = await this.isUsernameTaken(body.username, id);
      if (taken) {
  ...
  async createUserFromOpenId(openId: string) {
    ...
    await this.assignSequentialUsername(user.id);
  ...
  async assignSequentialUsername(userId: number) {
    const username = `user${userId}`;
    await this.userEntity.update({ id: userId }, { username });
    return username;
  }
