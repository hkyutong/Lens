...
export class ModelsService {
  ...
  private readonly writableModelFields = [
    'keyType',
    'modelName',
    'key',
    'status',
  ...
  private sanitizeModelPayload(params: Partial<SetModelDto>) {
    const source = (params || {}) as Record<string, any>;
    const payload: Record<string, any> = {};
    this.writableModelFields.forEach(field => {
      if (source[field] !== undefined) payload[field] = source[field];
    });
    const timeoutNum = Number(payload['timeout']);
    payload['timeout'] = Number.isFinite(timeoutNum) ? timeoutNum : null;
    return payload;
  }
  ...
  async setModel(params: SetModelDto) {
    try {
      const { id } = params;
      const payload = this.sanitizeModelPayload(params);
      if (id) {
        const res = await this.modelsEntity.update({ id }, payload);
        await this.initCalcKey();
        return res.affected > 0;
      } else {
        const keyType = Number(payload['keyType']);
        if (Number(keyType !== 1)) {
          const res = await this.modelsEntity.save(payload);
          await this.initCalcKey();
          return res;
        } else {
          const keyList = Array.isArray(params.key) ? params.key : [params.key];
          const data = keyList
            .filter(Boolean)
            .map(key => this.sanitizeModelPayload({ ...params, key: String(key) }));
          if (!data.length) {
            throw new HttpException('缺少可用的模型 key', HttpStatus.BAD_REQUEST);
          }
      ...
      throw new HttpException(error?.message || '设置模型失败', HttpStatus.BAD_REQUEST);
    }
  }
