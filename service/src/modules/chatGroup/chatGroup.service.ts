...
export class ChatGroupService {
  ...
  private forceVectorParsing(modelConfig: any) {
    if (modelConfig?.modelInfo) {
      modelConfig.modelInfo.isFileUpload = 2;
    }
  }

  private normalizeModelConfig(modelConfig: any) {
    if (!modelConfig?.modelInfo) return;
    this.forceVectorParsing(modelConfig);
    if (!this.globalConfigService.isNetworkSearchEnabled()) {
      modelConfig.modelInfo.isNetworkSearch = false;
    }
  }

  private applyVectorParsingToConfig(config: string) {
    if (!config) return config;
    try {
      const parsed = JSON.parse(config);
      this.normalizeModelConfig(parsed);
  ...
  async create(body: CreateGroupDto, req: Request) {
    if (!req?.user || req.user.role === 'visitor' || !req.user.id) {
      throw new HttpException('请登录后再新建对话', HttpStatus.UNAUTHORIZED);
    }
    ...
    this.normalizeModelConfig(modelConfig);
  ...
  async query(req: Request) {
    try {
      ...
      return res.map(item => ({
        ...item,
        config: this.applyVectorParsingToConfig(item.config),
      }));
  ...
  async update(body: any, req: Request) {
    ...
    if (config) {
      data['config'] = this.applyVectorParsingToConfig(config);
    }
  ...
  async getGroupInfoFromId(id) {
    ...
    if (groupInfo) {
      ...
      if (rest.config) {
        rest.config = this.applyVectorParsingToConfig(rest.config);
      }
      return rest;
    }
  }
