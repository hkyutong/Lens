...
export class BadWordsService implements OnModuleInit {
  ...
  private static readonly BAD_WORD_MAX_LENGTH = 20;
  constructor(
  ...
  async updateBadWords(body: UpdateBadWordsDto) {
    ...
    if (word && word.length > BadWordsService.BAD_WORD_MAX_LENGTH) {
      throw new HttpException(
        `敏感词长度不能超过${BadWordsService.BAD_WORD_MAX_LENGTH}个字符`,
