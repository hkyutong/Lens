...
export class UserBalanceService {
  ...
  private resolveDeductKeys(deductionType: number | string) {
    const normalized = String(deductionType ?? '').toLowerCase();
    const aliasMap: Record<string, 1 | 2 | 3> = {
      '1': 1,
      model3: 1,
      model3count: 1,
      text: 1,
    ...
    const type = aliasMap[normalized];
    if (!type) {
  ...
  async deductFromBalance(userId, deductionType, amount, UseAmount = 0) {
    const { member, nonMember, token, countKey } = this.resolveDeductKeys(deductionType);
    const normalizedAmount = Number(amount || 0);
    const normalizedUseToken = Number(UseAmount || 0);

    await this.userBalanceEntity.manager.transaction(async manager => {
      const repo = manager.getRepository(UserBalanceEntity);
      const b = await repo
      ...
      const memberBalance = Number(b[member] || 0);
      const nonMemberBalance = Number(b[nonMember] || 0);
      let nextMember = memberBalance;
      let nextNonMember = nonMemberBalance;
      ...
      if (normalizedAmount >= 0) {
        ...
        nextMember -= deductFromMember;
        remaining -= deductFromMember;
        if (remaining > 0) {
          const deductFromNonMember = Math.min(nextNonMember, remaining);
          nextNonMember -= deductFromNonMember;
          remaining -= deductFromNonMember;
        }
        if (remaining > 0) {
          throw new HttpException('消费余额失败！', HttpStatus.BAD_REQUEST);
        }
      } else {
        // 负值表示回滚补偿，统一退回基础额度，避免覆盖会员到期策略。
        nextNonMember += Math.abs(normalizedAmount);
      }

      const updateBalance: Partial<UserBalanceEntity> & Record<string, number> = {
        [member]: nextMember,
        [nonMember]: nextNonMember,
        [token]: Number(b[token] || 0) + normalizedUseToken,
      };

      if (countKey && normalizedAmount > 0) {
        const countField = String(countKey);
        updateBalance[countField] = Number((b as any)[countField] || 0) + normalizedAmount;
      }

      const result = await repo.update({ userId }, updateBalance);
