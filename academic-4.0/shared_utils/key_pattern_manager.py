...
def select_api_key(keys, llm_model):
    ...
    # 对未知模型名按 OpenAI 兼容协议兜底（用于动态模型管理场景）。
    # 明确前缀的请求源仍走原有分支，避免误选。
    provider_prefixes = ('api2d-', 'azure-', 'cohere-', 'openrouter-')
    use_openai_key_by_default = (
        ...
        or (not llm_model.startswith(provider_prefixes))
    )

    if use_openai_key_by_default:
        for k in key_list:
