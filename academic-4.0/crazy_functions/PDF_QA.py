...
def 解析PDF(file_name, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt):
    ...
    i_say_show_user = f'首先你在英文语境下通读整篇论文。'; gpt_say = "收到，开始分析。"           # 用户提示
    ...
    for i in range(n_fragment):
        ...
        i_say = (
            f"Read this section, recapitulate the content of this section with less than {NUM_OF_WORD} words: {paper_fragments[i]}\n"
            "请使用Markdown格式，至少包含“要点”和“关键证据”两个小节，每个小节使用列表，段落之间保留空行。"
        )
    ...
    final_results.append(
        ...
        "输出必须使用Markdown格式，使用清晰小标题+列表，不要输出单段大段落。"
    )
    # 接下来两句话只显示在界面上，不起实际作用
    i_say_show_user = f'接下来，你是一名专业的学术教授，利用以上信息，使用中文回答我的问题。'; gpt_say = "收到。"
