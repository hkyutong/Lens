...
def 解析docx(file_manifest, project_folder, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt):
    ...
    for index, fp in enumerate(file_manifest):
        ...
        for i, paper_frag in enumerate(paper_fragments):
            i_say = (
                f'请对下面的文章片段用中文做概述，文件名是{os.path.relpath(fp, project_folder)}，'
                f'文章内容是 ```{paper_frag}```。请使用Markdown格式分点输出，并保留段落空行。'
            )
            i_say_show_user = f'请对下面的文章片段做概述：{os.path.basename(fp)} 的第{i+1}/{len(paper_fragments)}个片段。'
        ...
        if len(paper_fragments) > 1:
            i_say = (
                f"根据以上的对话，总结文章《{os.path.basename(fp)}》的主要内容。"
                "请使用Markdown格式输出，不要使用表格，"
                "至少包含“核心结论/关键要点/待关注问题”三个小节，每个小节分点陈述。"
            )
            gpt_say = yield from request_gpt_model_in_new_thread_with_ui_alive(
                inputs=i_say,
