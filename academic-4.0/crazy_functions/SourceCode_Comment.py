...
def 注释源代码(file_manifest, project_folder, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt):
    ...
    for index, fp in enumerate(file_manifest):
        ...
        i_say_show_user = prefix + f'[{index+1}/{len(file_manifest)}] 请用一句话对下面的程序文件做一个整体概述: {os.path.relpath(fp, project_folder)}'
        # 装载请求内容
    ...
    configured_workers = get_conf('DEFAULT_WORKER_NUM')
    try:
        configured_workers = int(configured_workers)
    except Exception:
        configured_workers = 8
    env_cap = os.getenv("LENS_ACADEMIC_MAX_WORKERS", "8")
    try:
        env_cap = int(env_cap)
    except Exception:
        env_cap = 8
    safe_workers = max(1, min(configured_workers, env_cap, len(file_manifest)))
    ...
    while True:
        ...
        done_cnt = sum(worker_done)
        running_cnt = max(0, len(worker_done) - done_cnt)
        progress_text = f"任务处理中，请稍候（已完成 {done_cnt}/{len(worker_done)}，处理中 {running_cnt}）"
        if progress_text != last_progress_text or all(worker_done):
            yield from update_ui_latest_msg(progress_text, chatbot=chatbot, history=history, delay=0)
...
def 注释Python项目(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_request):
    ...
    if os.path.exists(txt):
        project_folder = txt
        if os.path.isfile(project_folder) and os.path.isdir(project_folder + ".extract"):
            project_folder = project_folder + ".extract"
        elif os.path.isfile(project_folder):
