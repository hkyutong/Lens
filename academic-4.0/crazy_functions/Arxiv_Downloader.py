...
def extract_arxiv_id(raw: str):
    match = re.search(r'(\d{4}\.\d{4,5}|[a-z-]+/\d{7})', str(raw))
...
def download_arxiv_(url_pdf):
    ...
    paper_id_guess = extract_arxiv_id(url_pdf) or extract_arxiv_id(url_abs)
    try:
        title, other_info = get_name(_url_=url_abs)
    except Exception:
        title = f"[{paper_id_guess}]"
        other_info = {"year": "", "authors": "", "comment": "", "abstract": ""}
    ...
    r = requests.get(requests_pdf_url, proxies=proxies, timeout=20, headers=headers)
    if r.status_code != 200 or "application/pdf" not in r.headers.get("content-type", "").lower():
        if paper_id_guess:
            fallback_url = f"https://export.arxiv.org/pdf/{paper_id_guess}.pdf"
            r = requests.get(fallback_url, proxies=proxies, timeout=20, headers=headers)
    if r.status_code != 200 or "application/pdf" not in r.headers.get("content-type", "").lower():
        raise RuntimeError(f"pdf_download_failed status={r.status_code}")
...
def 下载arxiv论文并翻译摘要(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_request):
    ...
    CRAZY_FUNCTION_INFO = "Lens 学术插件：正在提取摘要并下载 PDF 文档……"
    import glob
    import os
    ...
    # 翻译摘要等
    i_say = (
        "请你阅读以下学术论文相关材料，提取摘要并翻译为中文。"
        "请使用Markdown格式输出，结构包含：论文标题、中文摘要、关键词。"
        f"材料如下：{str(info)}"
    )
    paper_name = os.path.basename(str(pdf_path or "论文.pdf"))
    i_say_show_user =  f'请你阅读以下学术论文相关的材料，提取摘要，翻译为中文。论文：{paper_name}'
    chatbot.append((i_say_show_user, "处理中，请稍候。"))
    yield from update_ui(chatbot=chatbot, history=history) # 刷新界面
    msg = '正常'
    # ** gpt request **
    # 单线，获取文章meta信息
