...
def descend_to_extracted_folder_if_exist(project_folder):
    ...
    """
    maybe_dir = sorted([f for f in glob.glob(f'{project_folder}/*') if os.path.isdir(f) and f.endswith('.extract')])
    if len(maybe_dir) == 0:
...
def Latex英文纠错加PDF对比(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_request):
    ...
    try:
        ...
        chatbot.append(["任务状态", "任务初始化失败，请稍后重试。"])
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    has_latex_env = has_pdflatex_available()
    ...
    if has_latex_env:
        ...
        success = False
        chatbot.append(("任务状态", "未检测到可用的 LaTeX 编译环境，已生成纠错后的 Tex 结果（压缩包）。"))
        yield from update_ui(chatbot=chatbot, history=history)
    ...
    if success:
        ...
        chatbot.append((f"已完成", '已输出纠错后的Tex结果（压缩包），请查收。'))
        yield from update_ui(chatbot=chatbot, history=history);
        time.sleep(1)
        promote_file_to_downloadzone(file=zip_res, chatbot=chatbot)
...
def Latex翻译中文并重新编译PDF(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_request):
    ...
    if txt.endswith('.pdf'):
        report_exception(chatbot, history, a="解析项目", b="检测到已有翻译好的 PDF 文档。")
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    ...
    if os.path.exists(txt):
        ...
        if txt == "": txt = '空空如也的输入栏'
        report_exception(chatbot, history, a="解析项目", b="找不到本地项目或无法处理。")
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    ...
    if has_latex_env:
        ...
        success = False
        chatbot.append(("任务状态", "未检测到可用的 LaTeX 编译环境，已生成翻译后的 Tex 结果（压缩包）。"))
        yield from update_ui(chatbot=chatbot, history=history)
    ...
    if success:
        ...
        chatbot.append((f"已完成", '已输出翻译后的Tex结果（压缩包），请查收。'))
        yield from update_ui(chatbot=chatbot, history=history)
        time.sleep(1)
        promote_file_to_downloadzone(file=zip_res, chatbot=chatbot)
...
def PDF翻译中文并重新编译PDF(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    try:
        ...
        chatbot.append(["任务状态",
                        "未检测到可用的 LaTeX 编译环境，当前任务无法继续。请联系管理员完成环境配置后重试。"])
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    ...
    if len(file_manifest) == 0:
        report_exception(chatbot, history, a="解析项目", b="找不到任何 .pdf 文件。")
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    if len(file_manifest) != 1:
        report_exception(chatbot, history, a="解析项目", b="暂不支持同时处理多个 PDF 文件。")
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    ...
    chatbot.append(["任务状态", "正在将 PDF 转换为 TeX 项目，请稍候。"])
    yield from update_ui(chatbot=chatbot, history=history)
    project_folder = pdf2tex_project(file_manifest[0], plugin_kwargs)
    if project_folder is None:
        report_exception(chatbot, history, a="解析项目", b="PDF 转换为 TeX 项目失败。")
        yield from update_ui(chatbot=chatbot, history=history)
        return False
