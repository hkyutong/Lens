    ...
    for s in reversed([q for q in re.finditer(r"\\input\{(.*?)\}", main_file, re.M)]):
        ...
        if fp_:
            try:
                ...
                c = f"\n\nWarning from Lens ｜昱镜: LaTex source file is missing!\n\n"
        else:
...
\begin{abstract}
The Lens program cannot find abstract section in this paper.
\end{abstract}
"""
...
def _merge_pdfs_ng(pdf1_path, pdf2_path, output_path):
    import pypdf
    from pypdf.generic import NameObject, TextStringObject, ArrayObject, FloatObject, NumberObject

    def build_blank_page(reader, fallback_reader):
        if len(reader.pages) > 0:
            base_page = reader.pages[0]
        elif len(fallback_reader.pages) > 0:
            base_page = fallback_reader.pages[0]
        else:
            return pypdf.PageObject.create_blank_page(width=595, height=842)
        return pypdf.PageObject.create_blank_page(
            width=float(base_page.mediabox.width),
            height=float(base_page.mediabox.height),
        )

    Percent = 1
    # raise RuntimeError('pypdf has a serious memory leak problem, please use other tools to merge PDF files.')
    # Open the first PDF file
    with open(pdf1_path, "rb") as pdf1_file:
        pdf1_reader = pypdf.PdfReader(pdf1_file)
        # Open the second PDF file
        with open(pdf2_path, "rb") as pdf2_file:
            pdf2_reader = pypdf.PdfReader(pdf2_file)
            ...
            for page_num in range(num_pages):
                ...
                if page_num < len(pdf1_reader.pages):
                    ...
                    page1 = build_blank_page(pdf1_reader, pdf2_reader)
                # Add the page from the second PDF file
                if page_num < len(pdf2_reader.pages):
                    page2 = pdf2_reader.pages[page_num]
                else:
                    page2 = build_blank_page(pdf2_reader, pdf1_reader)
                ...
                new_page = pypdf.PageObject.create_blank_page(
                    width=int(
                        int(page1_width)
                        + int(page2_width) * Percent
                    ),
                    height=max(page1_height, page2_height),
                )
...
merge_pdfs = run_in_subprocess(_merge_pdfs)  # pypdf这个库有严重的内存泄露问题，把它放到子进程中运行，从而方便内存的释放
