...
def normalize_brand_text(text: str) -> str:
    ...
    if not value:
        return value
    value = value.replace("Lens Report ｜昱镜报告", "Lens Report｜昱镜报告")
    value = value.replace("# GPT-Academic Report", "# Lens Report｜昱镜报告")
    value = value.replace("GPT-Academic Report", "Lens Report｜昱镜报告")
    value = value.replace("GPT-Academic", "Lens")
    return value


def strip_plugin_contributor_text(text: str) -> str:
    value = str(text or "")
    if not value:
        return value
    # 去掉“函数插件贡献者/作者”品牌外显文本，仅保留正文内容。
    value = re.sub(
    ...
    value = re.sub(r"(?:函数)?插件(?:作者|贡献者)\s*\[[^\]]*][，,;；。\s]*", "", value, flags=re.IGNORECASE)
    return value


def normalize_local_message_prefix(text: str) -> str:
    value = str(text or "")
    if not value:
        return value
    return re.sub(r"\[\s*Local\s+Message\s*]\s*", "", value, flags=re.IGNORECASE)


def dedupe_token_truncation_warning_lines(text: str) -> str:
    value = str(text or "")
    if not value:
        return value
    lines = value.split("\n")
    out = []
    first_warning_pos = None
    max_overflow = None
    pattern = re.compile(
        r"^\s*警告，文本过长将进行截断，Token溢出数[:：]\s*(\d+)(?:\s*[，,]\s*现继续运行)?\s*[。.]?\s*$"
    )
    for line in lines:
        ...
        if not match:
            out.append(line)
            continue
        if first_warning_pos is None:
            first_warning_pos = len(out)
        overflow_raw = str(match.group(1) or "").strip()
        if not overflow_raw:
            continue
        try:
    ...
    if max_overflow is not None:
        warn = f"警告，文本过长将进行截断，Token溢出数：{max_overflow}，现继续运行。"
        if first_warning_pos is None or first_warning_pos < 0:
            out.insert(0, warn)
        else:
            out.insert(min(first_warning_pos, len(out)), warn)
    return "\n".join(out)


def strip_traceback_runtime_noise(text: str) -> str:
    value = str(text or "")
    if not value:
        return value

    value = re.sub(
        r"```(?:text|txt|log|python)?\s*\n[\s\S]*?Traceback[\s\S]*?```",
        "",
        value,
        flags=re.IGNORECASE,
    )
    value = re.sub(
        r"Traceback\s*\(most recent call last\):[\s\S]*?(?=\n\s*(?:重试中，请稍等|以下|总体而言|总之|摘要[:：]|论文标题[:：]|结论[:：]|总结[:：]|最终|收到|文件已生成|学术服务|$))",
        "",
        value,
        flags=re.IGNORECASE,
    )
    value = re.sub(r"^\s*File\s+\"[^\"\n]+\"\s*,\s*line\s+\d+[^\n]*$", "", value, flags=re.IGNORECASE | re.MULTILINE)
    value = re.sub(
        r"^\s*(?:The above exception was the direct cause of the following exception|During handling of the above exception, another exception occurred)\s*:?\s*$",
        "",
        value,
        flags=re.IGNORECASE | re.MULTILINE,
    )
    value = re.sub(r"^\s*(?:urllib3|requests|http\.client)\.[^\n]*$", "", value, flags=re.IGNORECASE | re.MULTILINE)
    value = re.sub(
        r"^\s*(?:urllib3\.exceptions\.[^\n]+|requests\.exceptions\.[^\n]+|http\.client\.[^\n]+)\s*$",
        "",
        value,
        flags=re.IGNORECASE | re.MULTILINE,
    )
    value = re.sub(
        r"^\s*(?:BrokenPipeError|ProtocolError|ConnectionError|ProxyError|MaxRetryError)\b[^\n]*$",
        "",
        value,
        flags=re.IGNORECASE | re.MULTILINE,
    )
    ...
    value = re.sub(
        r"^\s*(?:\[?\s*Local\s+Message\s*]?\s*)?(?:警告[，,\s]*)?(?:在执行过程中遭遇问题.*Traceback|Traceback)\s*[:：]?\s*$",
        "",
        value,
        flags=re.IGNORECASE | re.MULTILINE,
    )
...
def strip_plugin_runtime_notice_text(text: str) -> str:
    value = str(text or "")
    if not value:
        return value
    sentence_patterns = [
        r"对整个Latex项目进行(?:润色|纠错|翻译)[^。!！？\n]*(?:[。!！？]|$)",
        r"将PDF转换为Latex项目[^。!！？\n]*(?:[。!！？]|$)",
        r"分析结果[:：][^。!！？\n]*(?:[。!！？]|$)",
        r"主程序即将开始[^。!！？\n]*(?:[。!！？]|$)",
        r"正在精细切分latex文件[^。!！？\n]*(?:[。!！？]|$)",
    ...
    value = re.sub(r"GPT结果已输出\s*[，,]?\s*即将编译PDF", "", value, flags=re.IGNORECASE)
    value = re.sub(r"^\s*注意事项[:：]?\s*$", "", value, flags=re.IGNORECASE | re.MULTILINE)
    return value


def strip_multithread_status_noise(text: str) -> str:
    value = str(text or "")
    if not value:
        return value
    # 兼容流式“覆盖消息”被误当作“增量拼接”时形成的进度串联噪声。
    value = re.sub(
        r"(?:任务处理中\s*[，,]?\s*请稍候\s*[（(][^）)\n]{0,240}[）)]\s*)+",
        "",
        value,
    ...
    value = re.sub(r"\b任务处理中\s*[，,]?\s*请稍候\b", "", value, flags=re.IGNORECASE)
    value = re.sub(r"^\s*请开始多线程操作[。.]?\s*$", "", value, flags=re.IGNORECASE | re.MULTILINE)
    value = re.sub(
        r"^\s*多线程操作已经开始[，,:：]\s*完成情况[：:].*$",
        "",
        value,
        flags=re.IGNORECASE | re.MULTILINE,
    )
    ...
    status_line_pattern = re.compile(
        r"^\s*`?(?:等待中|执行中|已成功|截断重试|已失败|输入过长已放弃|等待重试\s*\d+|重试中\s*\d+/\d+|重试中，请稍等\s*\d+/\d+)`?(?:\s*[:：].*)?\s*$",
        flags=re.IGNORECASE,
    )
    inline_progress_pattern = re.compile(r"^\s*执行中\s*[:：]\s*\[.*\]\s*$", flags=re.IGNORECASE)
    for line in lines:
        if status_line_pattern.match(line or ""):
            continue
        if inline_progress_pattern.match(line or ""):
            continue
...
def sanitize_download_filename(name: str) -> str:
    ...
    if not value:
        return value
    value = re.sub(r"GPT-Academic\s*Report", "Lens-Report", value, flags=re.IGNORECASE)
    value = re.sub(r"GPT-Academic", "Lens-Report", value, flags=re.IGNORECASE)
    value = normalize_brand_text(value)
    value = value.replace("Lens Report｜昱镜报告", "Lens-Report")
    value = value.replace("LensReport｜昱镜报告", "Lens-Report")
    value = re.sub(r"\bLens\s+Report\b", "Lens-Report", value, flags=re.IGNORECASE)
    return value


def _replace_path_with_filename(match: re.Match) -> str:
    ...
    if not core:
        return raw
    normalized = core.replace("\\", "/").rstrip("/")
    file_like = re.findall(r"[^/]+?\.[A-Za-z0-9]{1,10}", normalized)
    if file_like:
        return file_like[-1]
...
def redact_sensitive_path_text(text: str) -> str:
    value = str(text or "")
    if not value:
        return value
    patterns = [
        r"(?:gpt_log|private_upload|downloadzone|public/file|file/dev/userFiles|userFiles|private/upload)/[^\n]*?\.[A-Za-z0-9]{1,10}",
        r"academic-4\.0/[^\n]*?\.[A-Za-z0-9]{1,10}",
        r"/Users/[^\n]*?\.[A-Za-z0-9]{1,10}",
        r"/(?:home|root|opt|var|tmp|www/wwwroot)/[^\n]*?\.[A-Za-z0-9]{1,10}",
        r"[A-Za-z]:\\[^\n]*?\.[A-Za-z0-9]{1,10}",
        r"(?:private_upload|public/file|userFiles|file/dev/userFiles|private/upload)/[^\s,，)]+",
        r"(?:gpt_log|downloadzone)/[^\s,，)]+",
        r"academic-4\.0/[^\s,，)]+",
        r"/Users/[^\s,，)]+",
        r"/(?:home|root|opt|var|tmp|www/wwwroot)/[^\s,，)]+",
        r"[A-Za-z]:\\[^\s,，)]+",
    ]
    for pattern in patterns:
        value = re.sub(pattern, _replace_path_with_filename, value, flags=re.IGNORECASE)

    value = (
        value.replace("【文件路径已隐藏】", "")
        .replace("[路径已隐藏]", "")
        .replace("[文件路径已隐藏]", "")
    )
    value = re.sub(r"(找不到任何[^\n:：]*文件)\s*[:：]\s*[^\n]*", r"\1。", value)
    value = re.sub(r"(找不到本地项目或(?:无权访问|无法处理))\s*[:：]\s*[^\n]*", r"\1。", value)
    value = re.sub(r"(解析项目)\s*[:：]\s*[^\n]*", r"\1", value)
    value = re.sub(r"\n{4,}", "\n\n\n", value)
    return value
...
def sanitize_report_text(text: str) -> str:
    ...
    if not value:
        return value
    value = strip_traceback_runtime_noise(value)
    value = normalize_brand_text(value)
    value = normalize_local_message_prefix(value)
    value = strip_plugin_contributor_text(value)
    value = strip_plugin_runtime_notice_text(value)
    value = strip_multithread_status_noise(value)
    value = redact_sensitive_path_text(value)
    value = re.sub(r"^\s*函数插件功能[？?]\s*$", "", value, flags=re.IGNORECASE | re.MULTILINE)
    value = re.sub(r"\b(?:PDF_Summary|Word_Summary|PDF_QA)\s*[。.:：]\s*", "", value, flags=re.IGNORECASE)
    value = dedupe_token_truncation_warning_lines(value)
    value = re.sub(r"\n{4,}", "\n\n\n", value)
    return value.strip() if value.strip() else ""


def sanitize_report_file_if_text(file_path: str):
    ext = os.path.splitext(str(file_path or ""))[1].lower()
    if ext not in {".md", ".markdown", ".txt", ".log"}:
        return
    ...
    try:
        ...
        return
    sanitized = sanitize_report_text(original)
...
def sanitize_chatbot_messages(chatbot: ChatBotWithCookies):
    for index, item in enumerate(chatbot):
        if not isinstance(item, (list, tuple)) or len(item) < 2:
            continue
        updated = list(item)
        left, right = updated[0], updated[1]
        if isinstance(left, str):
            left = normalize_brand_text(normalize_local_message_prefix(left))
            left = redact_sensitive_path_text(left)
        if isinstance(right, str):
            right = sanitize_chatbot_text(right)
        updated[0], updated[1] = left, right
        chatbot[index] = updated
...
def update_ui(chatbot:ChatBotWithCookies, history:list, msg:str="正常", **kwargs):  # 刷新界面
    ...
    sanitize_chatbot_messages(chatbot)
...
def CatchException(f):
    ...
    def decorated(main_input:str, llm_kwargs:dict, plugin_kwargs:dict,
        ...
        try:
            ...
            chatbot_with_cookie[-1] = [chatbot_with_cookie[-1][0], "学术服务连接异常，请稍后重试。"]
...
def write_history_to_file(
    ...
    if file_fullname is None:
        if file_basename is not None:
            file_basename = normalize_brand_text(file_basename)
            file_basename = sanitize_download_filename(file_basename)
            file_fullname = pj(get_log_folder(), file_basename)
        else:
            file_fullname = pj(get_log_folder(), f"Lens-Report-{gen_time_str()}.md")
    os.makedirs(os.path.dirname(file_fullname), exist_ok=True)
    with open(file_fullname, "w", encoding="utf8") as f:
        f.write("# Lens Report｜昱镜报告\n")
...
def promote_file_to_downloadzone(file:str, rename_file:str=None, chatbot:ChatBotWithCookies=None):
    ...
    if file_already_in_downloadzone(file, user_path):
        ...
        target_name = sanitize_download_filename(rename_file or current_name)
        if target_name and target_name != current_name:
            target_path = pj(os.path.dirname(file), target_name)
            if os.path.exists(target_path):
                target_path = pj(os.path.dirname(file), f"{gen_time_str()}-{target_name}")
        ...
        rename_file = sanitize_download_filename(rename_file)
    ...
    sanitize_report_file_if_text(new_path)
...
def to_markdown_tabs(head: list, tabs: list, alignment=":---:", column=False, omit_path=None):
    ...
    for i in range(max_len):
        ...
        if omit_path is not None:
            normalized_row_data = []
            for dat in row_data:
                if isinstance(dat, str) and os.path.exists(dat):
                    try:
                        normalized_row_data.append(os.path.relpath(dat, omit_path))
                    except Exception:
                        normalized_row_data.append(os.path.basename(dat))
                else:
                    normalized_row_data.append(dat)
...
def generate_file_link(report_files:List[str]):
    ...
    for f in report_files:
        display_name = os.path.basename(str(f))
        file_links += (
            f'<br/><a href="file={os.path.abspath(f)}" target="_blank">{display_name}</a>'
        )
    return file_links
    ...
    for saved_file in file_manifest:
        with open(new_pdf_path, 'rb') as file1, open(saved_file, 'rb') as file2:
            ...
            for page_num in range(len(reader1.pages)):
                page1 = reader1.pages[page_num].extract_text() or ""
                page2 = reader2.pages[page_num].extract_text() or ""
                if page1 != page2:
                    pages_same = False
                    break
