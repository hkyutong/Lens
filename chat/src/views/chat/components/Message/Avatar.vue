...
import gemini from '@/assets/aiavatar/gemini.gif'
...
const robotAvatar = computed(() => {
  const avatar = activeGroupInfo.value?.appLogo || authStore.globalConfig.robotAvatar
  return avatar
})
...
<template>
  ...
  <img
    ...
    class="inline-flex h-8 w-8 items-center justify-center rounded-full border da r border-gray-100 dark:border-gray-750 border-solid shadow-sm"
    :src="modelAvatar"
    alt="Robot Avatar"
  />
  <img
    v-else-if="isString(robotAvatar) && robotAvatar.length > 0"
    class="inline-flex h-8 w-8 items-center justify-center rounded-full border border-gray-100 dark:border-gray-750 border-solid shadow-sm"
    :src="robotAvatar"
    alt="Robot Avatar"
  />
  <img
  ...
  <span
    v-else
    class="select-none inline-flex h-8 w-8 items-center justify-center rounded-full border border-gray-200 dark:border-gray-700 border-solid dark:bg-gray-500 text-gray-600 dark:text-white font-semibold text-base"
  >
