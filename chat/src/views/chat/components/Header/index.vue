...
const guestModelLabel = computed(() => {
  if (modelOptions.value.length > 0) {
    return modelOptions.value[0]?.label || '未配置模型'
  }
  return '未配置模型'
})
...
const filteredModelOptions = computed(() => modelOptions.value)
/* 当前对话组是否是应用 */
...
async function queryModelsList() {
  try {
    ...
    const preferredOnly = flatModelArray.filter(model => model.keyType === 1)
    const modelsToUse = preferredOnly.length ? preferredOnly : flatModelArray
    ...
    if (chatStore.preferredModel?.value) {
      const exists = modelOptions.value.some(
        option => option.value === chatStore.preferredModel.value
      )
        ...
        <div class="flex justify-between items-center h-full w-full">
          ...
          <div v-else-if="notSwitchModel" class="flex-1 flex items-center">
            <div class="menu menu-md relative">
              <button
                class="menu-trigger flex items-center rounded-full border border-[#f2f2f2] bg-white px-4 py-2 text-sm font-medium text-[#080808] shadow-sm dark:border-white/10 dark:bg-[#080808] dark:text-white"
                aria-label="当前对话"
                disabled
              >
          ...
          <div v-else class="flex-1 flex items-center">
            <DropdownMenu v-model="isMenuOpen" position="bottom-left" max-height="40vh">
              <template #trigger>
                <button
                  class="menu-trigger flex items-center rounded-full border border-[#f2f2f2] bg-white px-4 py-2 text-sm font-medium text-[#080808] shadow-sm hover:bg-[#f2f2f2] dark:border-white/10 dark:bg-[#080808] dark:text-white dark:hover:bg-white/10"
              ...
              <template #menu="{ close }">
                <div class="min-w-[16rem] max-w-[22rem]">
                  <div
                    ...
                    class="px-4 pb-4 text-sm text-gray-500 dark:text-gray-400"
                  >
                    未找到匹配模型
                  </div>
                  <div
                    v-for="(option, index) in filteredModelOptions"
                    :key="index"
