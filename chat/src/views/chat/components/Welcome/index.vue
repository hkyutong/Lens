...
// 获取用户昵称
const nickname = computed(() => (authStore.userInfo as any)?.username || '')
// 根据时间获取问候语
...
const homeWelcomeContent =
  ...
  '一个免费的、原生支持 LaTeX 的科研工作区，集成智能协作与学术写作辅助。'
...
<template>
  <div v-if="activeAppId" class="flex flex-col justify-center items-center select-none">
    <div class="flex items-center mb-2">
      ...
      <h1 class="text-3xl font-bold text-[#080808] dark:text-white">{{ appDetail?.name }}</h1>
    </div>
  ...
  <div v-else class="flex flex-col items-center justify-center select-none">
    <div class="flex items-center">
      ...
      <h1 class="text-3xl font-bold text-[#080808] dark:text-white">{{ greeting }}</h1>
    </div>
