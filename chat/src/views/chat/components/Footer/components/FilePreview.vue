...
interface Props {
  ...
  showSavedFiles?: boolean // 是否展示已保存文件
...
<template>
  <div
    v-if="fileList.length > 0 || (showSavedFiles && savedFiles.length > 0) || isSelectedApp"
  ...
  >
    ...
    <div class="self-start w-full rounded-t-2xl mt-2">
      ...
      <div class="flex flex-wrap px-2">
        <!-- 已保存的文件显示 -->
        <template v-if="showSavedFiles && savedFiles.length > 0">
          <!-- 先显示图片 -->
          <div
        ...
        <template
          ...
          :key="'item-' + index"
        >
          <div v-if="file?.type.startsWith('image/')" class="relative inline-block mr-2 mb-2">
            <img
              :src="dataBase64List[index]"
          ...
          <div v-else class="relative inline-block mr-2 mb-2">
            <div
              class="px-3 flex items-center justify-start rounded-xl h-10 text-gray-700 dark:text-gray-400 border border-gray-100 shadow-sm dark:border-gray-700 transition-colors relative"
            >
              <span class="text-gray-500 max-w-48 truncate mr-4">{{ file.name }}</span>
